...
class PermisoDAO:
    ...
    @classmethod
    def tiene_permiso(cls, usuario_id, modulo, requiere_editar=False):
    ...
    @classmethod
    def asignar_permiso(cls, usuario_id, modulo, puede_ver=True, puede_editar=False):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO permisos_usuario (usuario_id, modulo, puede_ver, puede_editar)
                VALUES (%s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE 
                    puede_ver = VALUES(puede_ver),
                    puede_editar = VALUES(puede_editar)
            """, (usuario_id, modulo, puede_ver, puede_editar))
            conn.commit()
            return True
        except Exception as e:
    ...
    def asignar_todos_permisos(cls, usuario_id):
        ...
        modulos = ['inicio', 'tareas', 'campanas', 'calendario', 'influencers',
                   'ecommerce', 'reportes', 'usuarios']

        for modulo in modulos:
            cls.asignar_permiso(usuario_id, modulo, puede_ver=True, puede_editar=True)
